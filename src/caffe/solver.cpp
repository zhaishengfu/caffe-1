...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  for (; iter_ < param_.max_iter(); ++iter_) {
    ...
    Dtype loss;
    //std::vector<Blob<Dtype>*> bottom_vec; // dummy.
    net_->Forward(bottom_vec,&loss);
    Dtype loss3;
    //bottom_vec.clear();
    net_->Forward(bottom_vec,&loss3);
    Dtype loss1 = net_->ForwardBackward(bottom_vec);
    Dtype loss2 = net_->ForwardBackward(bottom_vec);
    if (display) {
      LOG(INFO) << "Iteration " << iter_ << ", loss = " << loss << ", loss1 = " << loss1 << ", loss2 = " << loss2 << ", loss3 = " << loss3;
    }
...
void SGDSolver<Dtype>::ComputeUpdateValue() {
  ...
  case Caffe::CPU:
    ...
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      //std::cout << "count=" << net_params[param_id]->count() << std::endl;
      // copy
